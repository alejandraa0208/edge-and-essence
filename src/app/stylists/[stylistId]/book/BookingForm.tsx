...
type CreateBookingOk = { ok: true; booking: { id: string } };
...
export default function BookingForm({ stylistId, stylistName, services }: {
  ...
  // ── Shared redirect after booking ─────────────────────────────────────────
  function handleBookingSuccess(bookingId: string) {
    router.push(`/admin/bookings/confirmed?bookingId=${bookingId}`);
  }
  ...
  if (step === 2) return (
    <StepShell step={2} title="Pick a date & time" subtitle={`${primaryService?.name ?? "Service"} · ${totalMinutes} min`}>
      ...
      <div style={{ marginTop: 20 }}>
        {!dayDate && <div style={{ opacity: 0.5, fontSize: 14 }}>Select a date above to see available times.</div>}
        {dayDate && loadingSlots && <div style={{ opacity: 0.6, fontSize: 14 }}>Loading times…</div>}
        {dayDate && slotsError && <div style={{ color: "#fca5a5", fontWeight: 800 }}>{slotsError}</div>}
  ...
  return (
    <StepShell step={3} title="Your info + deposit" subtitle={`${primaryService?.name ?? ""} · ${dayDate} at ${selectedTime}`}>
      ...
      <div style={{ marginTop: 22, border: "1px solid #1e293b", borderRadius: 12, padding: 16 }}>
        <div style={{ fontWeight: 900, marginBottom: 4 }}>Appointment reminders</div>
        <div style={{ fontSize: 13, opacity: 0.55, marginBottom: 14 }}>
          We'll send email reminders 1 week, 24 hours, and 2 hours before your appointment.
        </div>
        <label style={{
          display: "flex", alignItems: "flex-start", gap: 12, padding: "12px 14px", borderRadius: 10,
          border: `1px solid ${reminderOptIn ? "#60a5fa" : "#1e293b"}`,
          background: reminderOptIn ? "rgba(96,165,250,0.06)" : "#0f1a2e",
          ...
          />
          <div>
            <div style={{ fontWeight: 800, fontSize: 14 }}>Send me email reminders</div>
            <div style={{ fontSize: 12, opacity: 0.55, marginTop: 2 }}>
              Reminders go to {clientEmail || "your email address"}
            </div>
          </div>
        </label>
      </div>
      ...
      {/* No-show policy */}
      <div style={{ marginTop: 14, padding: "14px 16px", borderRadius: 10, border: "1px solid #1e293b", background: "#0f1a2e" }}>
        <div style={{ fontWeight: 800, fontSize: 14, marginBottom: 4 }}>No-show & late cancellation policy</div>
        <div style={{ fontSize: 12, opacity: 0.55, lineHeight: 1.5 }}>
          Cancellations within 24 hours or no-shows may be charged up to the full service amount using the card on file. By completing this booking you agree to this policy.
        </div>
      </div>
          ...
          <NoDepositConfirm
            ...
            onConfirm={async () => {
              try {
                ...
                handleBookingSuccess(json.booking.id);
              } catch (e) {
...
function StripePayAndBook(props: {
  ...
  const handlePayAndBook = async () => {
    try {
      ...
      const json = (await r.json()) as CreateBookingOk | CreateBookingErr;
      if (!r.ok || "error" in json) throw new Error("error" in json ? json.details || json.error : `HTTP ${r.status}`);
      props.onSuccess(json.booking.id);
    } catch (e) {
